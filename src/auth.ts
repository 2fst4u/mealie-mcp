...
class OAuthTokenProvider implements TokenProvider {
  ...
  private async fetchToken(): Promise<string> {
    ...
    const safeUrlStr = safeUrl(this.oauth.tokenUrl);
    ...
    try {
      ...
      throw new Error(`OAuth token request to ${safeUrlStr} failed: ${reason}`);
    } finally {
      clearTimeout(timer);
    }
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `OAuth token request to ${safeUrlStr} returned HTTP ${res.status} ${res.statusText}`,
      );
    }
    ...
    try {
      ...
      throw new Error(`OAuth token response from ${safeUrlStr} was not valid JSON.`);
    }
    if (!parsed || typeof parsed !== "object" || !parsed.access_token) {
      throw new Error(`OAuth token response from ${safeUrlStr} did not include an access_token.`);
    }
