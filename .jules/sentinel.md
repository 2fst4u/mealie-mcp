...
## 2024-05-18 - Potential memory exhaustion via unbounded file read in `http-client`
**Vulnerability:** The previous Node 18 fallback code buffered the entire contents of a local file using `readFile` prior to sending a multipart request, allowing potential memory exhaustion / V8 heap out-of-memory crashes on very large files (e.g. up to 50MB limits multiplied by concurrent requests).
**Learning:** `fetch` in Node (via Undici) accepts a duck-typed `Blob` implementation. We can create a custom `StreamBlob` object that implements `stream()`, `text()`, `arrayBuffer()`, `slice()`, and `[Symbol.toStringTag]` to wrap `fs.createReadStream` and prevent files from buffering in memory during upload.
**Prevention:** When sending files via `fetch`, prefer `fs.openAsBlob` if available, or a stream-backed duck-typed Blob implementation as a fallback for older Node environments. Avoid buffering large files with `readFile` before placing them into requests.
